...
class SFTPInterface(paramiko.SFTPServerInterface):
    ...
    def realpath_for_read(self, path):
        return self._realpath(path, self.user.has_read_access, False)

    def realpath_for_write(self, path, block_root=False):
        return self._realpath(path, self.user.has_write_access, block_root)

    def _realpath(self, path, permission_check, block_root=False):
        path = self.canonicalize(path).lstrip('/')
        if not permission_check(path, block_root):
    ...
    def rename(self, oldpath, newpath):
        old_real = self.realpath_for_write(oldpath, True)
        new_real = self.realpath_for_write(newpath, True)
    ...
    @log_event
    def rmdir(self, path):
        os.rmdir(self.realpath_for_write(path, True))
